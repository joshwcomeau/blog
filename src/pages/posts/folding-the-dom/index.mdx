      ...
      {interpolated => {
        ...
        return (
          <div>
            ...
            <img width={400} src={catsSrc} style={{ transform }} />
            <div
              style={{
                position: 'relative',
                zIndex: 2,
...
Next, we need to make sure that this div is only shown when the card is facing the viewer.

If you're like me, you're probably thinking of a JS solution, triggering `opacity` on a `setInterval` to show/hide it as the card rotates. Fortunately, though, CSS has an elegant solution for us, built right into the language.
...
We can fix this by being a little sneaky, and giving our backside element a 180-degree rotation. Think of it like stacking two playing cards, and then flipping the top one over, so that the two cards are facing each other. Now, when you rotate the entire stack, you're always seeing the backside of _one_ of the elements.

We can also apply a very slight Z-translate, to push the element a bit closer to the viewer than the card. This addresses an issue where the elements can appear to flicker, because both the card and the backdrop are occupying the same point in 3D space.
...
Spacial orientation is a hard thing to visualize (especially with nested rotations!), so don't be discouraged if it's not immediately obvious why this trick works.
...
This was accomplished with a `transform: translateY()` on the parent, using the percentage of opening as the value to tween based on. This did introduce a subtle flickering bug on Chrome, and the solution was to add a third copy of the image in the crook of the fold.
...
In the demo, I'm using React Spring to animate changes in value, when the slider is dragged. Spring physics produce much more organic, beautiful motion than using traditional easing. Its use is outside the scope of this article, but it has <TextLink href="react-spring.io/docs">excellent documentation</TextLink> you can consult!
