...
We can fix this by being a little sneaky, and giving our backside element a 180-degree rotation. Think of it like stacking two playing cards, and then rotating the top one 180 degrees, so that the two cards are facing each other. Now, when you rotate the entire stack, you're always seeing the backside of _one_ of the elements.

We can also apply a Z-translate, to push the element a bit closer to the viewer
than the card. This addresses an issue where the elements can appear to flicker, because both the card and the backdrop are occupying the same point in 3D space.
...
How do we fit this into our `<Foldable>` component? We have to do a tiny bit of restructuring. Here's what it looks like:
...
You can view the solution in the full source code.
...
See this line in the source.

### Thickness

As the card moves through the second half, there's the illusion of thickness, as if the card has an edge.

This little trick is done by pushing down the backside by a couple pixels. I discovered this quite accidentally, but liked the way it looked!
...
In this post, we looked exclusively at images, but you may wish to use this effect on other kinds of elements. For example, in a demo I created for a React Europe talk, I show how a form can be folded up like a letter:
