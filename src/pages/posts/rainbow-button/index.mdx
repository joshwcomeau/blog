...
One of the things I really like about hooks is that they let us come up with our own abstractions, grouped by category rather than by lifecycle. I'm a fan of the school of thought that code should be designed for humans to read, not computers. And hooks let us design our code for humans,

In this case, I want to focus on keeping things as straightforward and lightweight as possible. I don't care too much about reusability, and I don't need a bunch of configuration options.
...
The next thing I wanted to figure out was the hook's interface. I started by writing the component that will consume this hook. I like just making up whatever API seems ideal for the component that uses it. I'll worry about making it later.
...
There's one thing that makes my spidey-sense tingle a bit; it's pretty surprising that `useRainbow` secretly registers global CSS custom properties. In fact, registering a global value from within an instanced component is going to be problematic. Let's see what we can do about that.

## Making this production-ready

Before you start shipping rainbow buttons, there are a couple things we need to think about.
...
The browser support for this media query has gotted dramatically better, and now supports all major browsers except IE. We'll set it up so that we're only enabling animations for folks who have selected "no preference" on motion, the default value:

<Code lang="js">{prmCode}</Code>

For folks using older browsers like Internet Explorer, the media query won't exist, which means `prefersReducedMotion.matches` will always return `false`. That's OK: we can assume that folks using legacy browsers would prefer not to receive the animations.
