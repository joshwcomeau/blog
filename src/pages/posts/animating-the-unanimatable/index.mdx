...
The DOM is dumb. If an item is removed from its original location and re-inserted at a new location 200px down, it has no _awareness_ about what that update means for the element’s on-screen position.
...
The thing is, we’ve made it so that we’re in a situation where _regular CSS transitions can work again_. To animate these elements to their new position, we can just add a transition and undo the artificial position changes.
